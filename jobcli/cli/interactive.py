...
def _run_onboarding(force: bool = False):
    ...
    try:
        ...
        if force or not has_llm or not has_wbox or not has_resume:
            ...
            # ── Write ~/.jobcli/.env so os.getenv() callers (WboxDiscoverer, etc.) ──
            # can read the credentials without requiring the user to manually create a file.
            env_dir = os.path.join(os.path.expanduser("~"), ".jobcli")
            os.makedirs(env_dir, exist_ok=True)
            env_path = os.path.join(env_dir, ".env")

            # Build the key/value pairs to persist
            env_vars: dict[str, str] = {}

            # Read existing .env so we don't wipe unrelated keys
            ...
            # Overwrite with freshly collected values
            if db_config.job_board_username:
                env_vars["JOBCLI_USERNAME"] = db_config.job_board_username
            if db_config.job_board_password:
                env_vars["JOBCLI_PASSWORD"] = db_config.job_board_password
            if db_config.openai_api_key:
                env_vars["OPENAI_API_KEY"] = db_config.openai_api_key
            if db_config.anthropic_api_key:
                env_vars["ANTHROPIC_API_KEY"] = db_config.anthropic_api_key
            if db_config.gemini_api_key:
                env_vars["GEMINI_API_KEY"] = db_config.gemini_api_key
            if db_config.default_llm_provider:
                env_vars["DEFAULT_LLM_PROVIDER"] = db_config.default_llm_provider

            with open(env_path, "w", encoding="utf-8") as _ef:
                _ef.write("# W-BOX CLI — auto-generated by setup wizard. Do not commit.\n")
                for _k, _v in env_vars.items():
                    _ef.write(f"{_k}={_v}\n")

            # Also load them into the current process so discover works immediately
