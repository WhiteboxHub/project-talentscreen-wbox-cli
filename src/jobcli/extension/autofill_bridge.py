...
from jobcli.extension.helpers import is_likely_ats_frame_url, read_extension_manifest_version
...
_CONFIGURE_JS = """async (o) => await window.AutofillExtension.configure({
...
def _ordered_frames_for_probe(page: Page) -> list[Frame]:
    """ATS application frames first, then others."""
    try:
    ...
    for frame in all_frames:
        if is_likely_ats_frame_url(frame.url or ""):
...
def _frame_has_bridge(frame: Frame) -> bool:
    try:
        return bool(frame.evaluate(_BRIDGE_JS))
    except Exception:
        return False
...
def find_autofill_frame(page: Page) -> Optional[Frame]:
    ...
    for frame in _ordered_frames_for_probe(page):
        if _frame_has_bridge(frame):
            return frame
    return None


def extension_api_available(page: Page) -> bool:
    """Return True if the TalentScreen bridge is callable in any frame."""
...
def wait_for_autofill_api(page: Page, timeout_ms: int = 30_000) -> Optional[Frame]:
    ...
    try:
        page.wait_for_function(_BRIDGE_JS, timeout=min(timeout_ms, 10_000))
    except Exception:
        pass
...
def _evaluate_api(frame: Frame, expression: str, arg: Any = None) -> Any:
    """Run JS on *frame*; await Promises from the page-world bridge."""
    if arg is None:
...
def run_extension_autofill(
    ...
    try:
        out.report = _evaluate_api(
            frame, "async () => await window.AutofillExtension.exportReport()"
        )
    except Exception:
        out.report = None
