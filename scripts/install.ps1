...
$Branch        = if ($env:JOBCLI_BRANCH) { $env:JOBCLI_BRANCH } else { "mahi_dev2" }
...
if not exist "%JOBCLI_VENV%\Scripts\python.exe" (
    ...
    echo Re-install with: irm https://raw.githubusercontent.com/WhiteboxHub/wbox-cli/mahi_dev2/scripts/install.ps1 ^| iex
    exit /b 1
)
