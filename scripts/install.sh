...
BRANCH="${JOBCLI_BRANCH:-mahi_dev2}"  # Override with JOBCLI_BRANCH=<branch> if needed
...
if [ ! -f "$JOBCLI_VENV/bin/python" ]; then
    ...
    echo "Re-install with: curl -fsSL https://raw.githubusercontent.com/WhiteboxHub/wbox-cli/mahi_dev2/scripts/install.sh | bash"
    exit 1
fi
