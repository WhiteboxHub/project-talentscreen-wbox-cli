...
def test_is_likely_ats_frame_url():
    ...
    assert _is_likely_ats_frame_url("https://jobs.lever.co/spotify/apply") is True

    assert _is_likely_ats_frame_url("https://boards.greenhouse.io/co/jobs/1") is True

    assert _is_likely_ats_frame_url("https://newassets.hcaptcha.com/captcha/v1/static/hcaptcha.html") is False
...
def test_run_extension_autofill_calls_inject_configure_fill(sample_resume):
    ...
    frame.evaluate.side_effect = [
        ...
        True,  # probe in find_autofill_frame / wait
    ...
    first_call_arg = frame.evaluate.call_args_list[1][0][0]
...
def test_run_extension_autofill_api_unavailable(sample_resume):
    ...
    inject_calls = [
        ...
        for c in frame.evaluate.call_args_list

        if c[0] and "injectProfile" in c[0][0]
...
def test_run_extension_autofill_logs_extension_dir_on_miss(sample_resume):
    ...
    assert "pageWorldBridge" in warning_msg or "bridge" in warning_msg.lower()
